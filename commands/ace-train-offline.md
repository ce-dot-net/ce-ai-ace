...
Usage:
```bash
# Run 5 epochs on all available code
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/offline-training.py

# Run 3 epochs on test files only
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/offline-training.py --epochs 3 --source test-files

# Run on git history
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/offline-training.py --source git-history
```
